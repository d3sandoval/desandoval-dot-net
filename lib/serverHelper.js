  ...
  getBlogPosts(type, limit, callback, refetch) {
    ...
    this.client.get(type, (err, storedData) => {
      ...
      if(!refetch && storedData) {
        let parsedData = storedData.toString();
      ...
      this.fetchBlogPosts(type, (err, fetched) => {
        ...
        const entries = (type === 'blog') ? fetched.response : fetched.feed.entries;
        ...
        for(let i=0; i<limit; i++){
          ...
          const isoDate = new Date(
            (type === 'blog') ? entries[i].firstPublishedAt : entries[i].isoDate
          );
    ...
    switch (type) {
      ...
      case 'blog':
        mediumJSONFeed('@d3sandoval/latest')
          ...
          .catch(function(data){callback(data)});
        break;
      default:
    ...
    dirs.forEach((dir) => {
      fs.stat(dir, (err, stats) => {
        ...
        this.client.hgetall(key, (err, storedData) => {
          ...
          let parsedData = (storedData !== null) ? storedData.toString() : null;
          if (!err) {
            this.isUpdated(
              key,
