...
class Logo extends React.Component {
    ...
    return (
        <React.Fragment>
            <Animated.div keyframes={this.getKeyFrames(size)}
            timing={this.getTiming(2000)}
            playState={this.state.playState}
            >
            <img alt="Click to enter" onClick={(this.state.hasAnimated) ? "" : this.handleClick} width={size} height={size} src="/img/logo.png" className={(this.state.hasAnimated) ? classes.after : classes.before } style={{
                marginTop: (size/4),
                marginLeft: -(size/2),
            }} />
            </Animated.div>
